...
class MLPHead(nn.Module):
    def __init__(self, norm_type, in_channels, mlp_hidden_size, projection_size, layer=1):
        super(MLPHead, self).__init__()
        # self.layer = layer
        # if self.layer==1:
        #     self.pool = nn.AdaptiveAvgPool2d((1,in_channels))
        if norm_type == 'batch':
            layer_norm = nn.BatchNorm1d(mlp_hidden_size)

        else:
            layer_norm = NormSelect('power', mlp_hidden_size, head_num=1)
