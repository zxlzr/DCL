...
def generate_checklist_perturb(origin_q):
    ...
    for perturb_func in [Perturb.contractions, Perturb.add_typos,
                         Perturb.strip_punctuation,Perturb.punctuation,
...
class GlueContrastDataset(Dataset):
    """
        ...
        with FileLock(lock_path):
            ...
            if os.path.exists(cached_features_file) and not args.overwrite_cache:
                ...
                if isinstance(examples[0],tuple):
                    for text_a, text_b in tqdm(examples):

                        words_raw_enhanced1 = text_a.split(' ')
                        words_raw_enhanced2 = text_b.split(' ')
                        # 用'. '来区分句子
                        ...
                        for j in range(check_num):
                            ...
                            batch_encoding_raw = np.array(tokenizer((sentences1+sentences2), add_special_tokens=True, truncation=True,
                    ...
                    for text_a in tqdm(examples):
                        ...
                        for j in range(check_num):
                            sentences1 = sentences1_checklist[j]
                            batch_encoding_raw = np.array(
                                tokenizer((sentences1 ), add_special_tokens=True, truncation=True,
                                          max_length=args.max_seq_length, padding="max_length")["input_ids"])
                            batch_encoding_enhances = np.array(
                                tokenizer((sentences1_enhanced ), add_special_tokens=True,
                                          truncation=True,
                                          max_length=args.max_seq_length, padding="max_length")["input_ids"])
                            self.features.append((batch_encoding_raw, batch_encoding_enhances))
