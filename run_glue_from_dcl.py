...
def main():
    ...
    # load contrast-trained parameters
    _, robertaformlm = prepare_bert(model_path=model_args.model_name_or_path)
    robertaformlm.resize_token_embeddings(len(tokenizer))
    online_network = BertNet(bert=robertaformlm,norm_type='power',model_name=model_args.model_name_or_path, **byol_config['network']) # data_args.task_name.upper()
    load_params = torch.load(os.path.join('/home/chenxiang/pretrained_model/PyTorch-BYOL/new_result/bert_power_pretrain_glue',data_args.task_name.upper(),'pytorch_model.bin'))
    if 'online_network_state_dict' in load_params:
       online_network.load_state_dict(load_params['online_network_state_dict'])
       model.bert.load_state_dict(online_network.roberta.state_dict())
       del online_network, load_params,robertaformlm
